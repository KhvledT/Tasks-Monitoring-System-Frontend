...
export const AppLayout: React.FC = () => {
  ...
  const { activeVessel, isArchiveMode, setViewedVessel } = useActiveVessel();
  ...
  return (
    <div className="min-h-screen bg-bg-light text-text-dark flex flex-col md:flex-row font-sans">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col w-64 border-r border-border-subtle bg-white p-5 shrink-0 justify-between">
        <div className="flex flex-col gap-6">
          ...
          {activeVessel && (
            <div className="mb-2 flex flex-col gap-2 px-2">
              <p className="text-[10px] uppercase font-bold text-zinc-450 tracking-wider">Vessel Workspace</p>
              <VesselSelector />
            </div>
          )}
        ...
        <div className="border-t border-border-subtle pt-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5 truncate">
            ...
            <div className="truncate flex flex-col gap-0.5">
              <p className="text-xs font-bold text-black truncate">{user?.fullName || 'Unnamed'}</p>
              <p className="text-[9px] font-bold text-zinc-400 uppercase tracking-wide truncate">{user?.rank || 'Officer'}</p>
            </div>
          </div>
          <button
            onClick={logout}
      ...
      <header className="md:hidden border-b border-border-subtle bg-white px-5 py-4 flex items-center justify-between shrink-0">
        <div className="flex flex-col">
          <span className="font-extrabold tracking-tight text-[#0055d4] text-base">Maritime Monitor</span>
          <span className="text-[8px] text-zinc-400 font-bold uppercase tracking-wider">Vessel Ops Center</span>
        </div>
      ...
      {/* Main Content Area */}
      <main className="flex-1 flex flex-col min-h-0 relative overflow-y-auto pb-20 md:pb-0 bg-bg-light">
        {/* Top Content Navigation Bar (matching screenshot layout) */}
        <div className="hidden md:flex items-center justify-between bg-white border-b border-border-subtle px-8 py-3.5 sticky top-0 z-30 select-none">
          <div className="flex items-center gap-3">
            <span className="text-lg font-bold text-[#0055d4] tracking-tight">
              {activeVessel?.name || 'MV Atlantic Star'}
            </span>
            <div className="w-px h-4 bg-zinc-200" />
            <div className="flex items-center gap-4 text-xs font-semibold text-zinc-400">
              <span className="hover:text-black cursor-pointer transition">Fleet View</span>
              <span className="hover:text-black cursor-pointer transition">Maintenance</span>
              <span className="hover:text-black cursor-pointer transition">Logistics</span>
            </div>
          </div>

          <div className="flex items-center gap-4 text-zinc-500">
            {/* Clock icon */}
            <span className="p-1 hover:bg-zinc-50 rounded-lg cursor-pointer transition" title="Timeline / Current Time">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="w-4.5 h-4.5">
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
              </svg>
            </span>
            {/* Alert / Notification bell */}
            <span className="p-1 hover:bg-zinc-50 rounded-lg cursor-pointer transition relative" title="Notifications">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.8} stroke="currentColor" className="w-4.5 h-4.5">
            ...
            <div className="w-7 h-7 rounded-full bg-blue-50 border border-primary/20 flex items-center justify-center font-bold text-[10px] text-primary shadow-inner cursor-pointer" title="Account settings">
              {getInitials(user?.fullName)}
            </div>
          </div>
        </div>
        ...
        <div className="p-6 md:p-8 flex-1 flex flex-col max-w-7xl w-full mx-auto animate-fade-in">
          {isArchiveMode && (
            <div className="bg-amber-50 border border-amber-250 p-4.5 rounded-xl flex items-center justify-between gap-4 mb-6 shadow-sm">
              <div className="flex items-center gap-3">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5 text-amber-600 shrink-0">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                </svg>
                <p className="text-xs font-semibold text-amber-800 leading-relaxed">
                  You are viewing historical data for this vessel. All actions are in read-only mode.
                </p>
              </div>
              <button
                type="button"
