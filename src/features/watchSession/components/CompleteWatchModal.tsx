...
interface CompleteWatchModalProps {
  isOpen: boolean;
  isLoading: boolean;
  errorMsg: string | null;
  onSubmit: (data: { signature: string; notes?: string }) => Promise<void>;
  onCancel: () => void;
}

export const CompleteWatchModal: React.FC<CompleteWatchModalProps> = ({
  isOpen,
  isLoading,
  errorMsg,
  onSubmit,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            <div className="flex items-center justify-between border-b border-zinc-900 pb-4 mb-5">
              <div>
                <h3 className="text-base font-bold text-zinc-100 tracking-tight">Complete Watch Shift</h3>
                <p className="text-xs text-zinc-400 mt-0.5">Sign watchkeeping logs and archive shift context.</p>
              </div>
              <button
                onClick={onCancel}
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                  Officer Digital Signature (Full Name)
                </label>
                <input
                  type="text"
                  required
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                  Closing Watch Remarks (Optional)
                </label>
                <textarea
                  placeholder="Log any final comments or watch summaries..."
                  value={notes}
              ...
              <div className="flex items-center gap-3 mt-4">
                ...
                <button
                  ...
                  className="flex-1 bg-emerald-500 hover:bg-emerald-400 disabled:bg-emerald-500/50 text-xs text-black font-bold py-2.5 rounded-xl transition flex items-center justify-center gap-2"
                >
                  {isLoading ? 'Completing...' : 'Complete & Seal Logs'}
                </button>
              </div>
            </form>
          </motion.div>
...
export default CompleteWatchModal;
