...
export const WatchSessionCockpit: React.FC<WatchSessionCockpitProps> = ({ vesselId }) => {
  ...
  const [elapsedParts, setElapsedParts] = useState({ h: '00', m: '00', s: '00' });
  ...
  useEffect(() => {
    ...
    if (params.get('startWatch') === 'true' && watch === null && !startMutation.isPending) {
  ...
  const handleStart = async () => {
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to start watch session.');
    }
  };
  ...
  const handlePause = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to pause watch session.');
    }
  };
  ...
  const handleResume = async () => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Failed to resume watch session.');
    }
  };
  ...
  const handleCompleteSubmit = async (data: { signature: string; notes?: string }) => {
    if (!watch) return;
    setModalError(null);
    try {
      ...
      <div className="w-full bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-4">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1 max-w-lg leading-relaxed">
            No active watchkeeper context is initialized for this vessel workspace. Initiate logs to record inspections and report machinery defects.
          </p>
        </div>
        <button
  ...
  const startTimeFormatted = watch.startedAt
    ? new Date(watch.startedAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', second: '2-digit', timeZone: 'UTC' })
    : '--:--:--';
    ...
    <div className="w-full bg-white border border-zinc-200 rounded-2xl p-6 shadow-sm flex flex-col gap-5 mb-4">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5">
        ...
        <div className="flex flex-col items-end gap-0.5">
          <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Elapsed Time</span>
          <span className="text-4xl font-extrabold text-black font-mono tracking-tighter">
            {elapsedParts.h}<span className="text-zinc-300">:</span>{elapsedParts.m}<span className="text-zinc-300">:</span>{elapsedParts.s}
          </span>
        </div>
      </div>
      ...
      {/* <div className="grid grid-cols-2 md:grid-cols-3 gap-4 border-t border-zinc-100 pt-4">
        <div>
          <span className="text-[9px] text-zinc-400 uppercase tracking-wider block mb-0.5">Watchkeeper</span>
          <span className="text-sm text-black">{watch.watchkeeper?.fullName || 'Current Officer'}</span>
        </div>
        <div>
          <span className="text-[9px]  text-zinc-400 uppercase tracking-wider block mb-0.5">Start Time (UTC)</span>
          <span className="text-sm text-black font-mono">{startTimeFormatted}</span>
        </div>
        <div>
          <span className="text-[9px] font-bold text-zinc-400 uppercase tracking-wider block mb-0.5">Relief Expected</span>
          <span className="text-smfont-bold text-black font-mono">--:--:--</span>
        </div>
      </div> */}
      ...
      <div className="flex flex-wrap items-center gap-3 border-t border-zinc-100 pt-4">
        {watch.status === 'PAUSED' ? (
          <button
            ...
            className="px-4 py-2.5 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-xs font-bold text-emerald-700 rounded-xl transition cursor-pointer"
          >
            Resume Walk
          </button>
        ) : (
          <button
            ...
            className="px-4 py-2.5 bg-white hover:bg-zinc-50 border border-zinc-200 text-xs font-bold text-gray rounded-xl transition cursor-pointer"
          >
            Log Event
          </button>
        )}
