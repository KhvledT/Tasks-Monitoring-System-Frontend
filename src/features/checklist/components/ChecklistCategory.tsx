...
export const ChecklistCategory: React.FC<ChecklistCategoryProps> = ({
  ...
  return (
    <div className="border border-zinc-900 bg-zinc-950/20 rounded-2xl overflow-hidden mb-4 shadow-sm hover:border-zinc-850/80 transition duration-150">
      {/* Accordion Header */}
      <button
        type="button"
        onClick={() => setIsOpen(!isOpen)}
        className="w-full flex items-center justify-between px-6 py-4 bg-zinc-950/60 border-b border-zinc-900/40 text-left cursor-pointer focus:outline-none"
      >
        <div className="flex items-center gap-3">
          <h4 className="text-sm font-bold text-zinc-100 tracking-tight">{category.name}</h4>
          <span
        ...
        <svg
          ...
          strokeWidth={2}
          stroke="currentColor"
          className={`w-4 h-4 text-zinc-500 transition-transform duration-250 ${
            isOpen ? 'rotate-180' : ''
          }`}
        >
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          <motion.div
            ...
            className="overflow-hidden bg-zinc-950/5"
          >
